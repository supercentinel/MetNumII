...
function diferencias_newton_reg(puntos::Array{Punto}, est::Float64, grado::Int64)
    ...
    for i ∈ 1:grado+1
        ...
        if i ≠ grado+1
            print(dif_tab[size(dif_tab, 1)-grado+i-1, grado-i+3], "/ ", grado-i+1, "! + ")
            ∑p += round(∏s, digits=6) * ((round(dif_tab[size(dif_tab, 1)-grado+i-1, grado-i+3], digits=6))/(factorial(grado-i+1)))
        else
            print(dif_tab[size(dif_tab, 1)-grado+i-1, grado-i+3])
